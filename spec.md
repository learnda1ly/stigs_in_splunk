...
| POST | `/stig_collections/{id}/jobs` | JSON `{operation: "archive_export", format, host_id?, baseline_id?}` | **201** async archive export job (`job_id`, `status`: `pending` → `running` → `succeeded` \| `failed`). Same export ACL as sync archive (workspace read; **404** when hidden). Creator-only `GET`/`DELETE` on the job (like baseline zip jobs). Staged zip under `$SPLUNK_HOME/var/run/stigs_in_splunk/collection_jobs/{job_id}/` (**6h TTL**, deleted on access after expiry). Poll `GET .../jobs/{jobId}`; download `GET .../jobs/{jobId}/download` returns the same JSON shape as sync archive when `status=succeeded`. Collection zip **import** remains synchronous on `POST .../imports`. |
