...
| GET/POST | `/stig_imports/review_aging_report` | Scheduled-search helper: stale reviews for all workspaces with `review_aging.enabled=true`. **Not grant-filtered** (like `inputlookup`); use collection stale GET for ACL-scoped views. Invoked by `\| stigkvreviewaging` and saved search **STIG review aging report** (disabled by default). |
