...
| Grant ACL (asset/STIG/label scoped) | UI: target icon on grant. API: `PUT .../grants/{grantId}/acl` | **done** | Host, baseline, and **label** id filters on restricted grants; enforced on REST list/get/patch, metrics/findings/unreviewed, and SplunkUI host lists. | `PUT/PATCH .../grants/{id}/acl`; restricted users see only allowed hosts/checklists/reviews/metrics (including label scope). | P1 | L |
| Labels on assets | UI: label assignment, filter. API: `/collections/{id}/labels`, `.../labels/{labelId}/assets` | **partial** | KV `stig_labels` + host `label_ids`; REST `GET/POST /stig_collections/{id}/labels`, `POST .../labels/{id}/assets`; `GET /stig_hosts?label_id=`; STIG Editor label filter. No dedicated label management UI (create via REST). | KV collection or embedded labels; filter `GET /stig_hosts` and editor by label. | P2 | M |
